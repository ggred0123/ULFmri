...
def main():
    ...
    def run_validation(step):
        ...
        lat = torch.randn((n, latent_ch, latent_hw, latent_hw), device=acc.device, dtype=weight_dtype)
        pe = null_prompt_embeds.repeat(n, 1, 1)
        pooled = null_pooled.repeat(n, 1)
        if avail_embedder is not None:
        ...
        if args.n_modalities > 1:
            ...
            img = torch.stack(chans, dim=1)                                # [n,3,H,W]
        else:
            img = val_vae.decode(lat).sample
        img = ((img + 1) / 2).clamp(0, 1).float().cpu().numpy()          # [n,3,H,W]
        grid = np.concatenate(
            [np.concatenate([img[i, c] for c in range(3)], axis=1) for i in range(n)], axis=0)
        ...
        if args.report_to == "wandb":
            import wandb
            acc.log({"samples": wandb.Image(fp, caption=f"step {step} — rows=samples, cols=T1|T2|FLAIR")},
                    step=step)
    ...
    while not done:
        for batch in loader:
            with acc.accumulate(transformer):
                ...
                pe = null_prompt_embeds.repeat(bsz, 1, 1)
